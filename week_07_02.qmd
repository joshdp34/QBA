# Week 7: Research Design, Bias, and Causality {#sec-week07-02}
...
## Observational Studies vs. Experiments, Revisited
...
::: {.example title="Example 7.3: A confounded association"}
...
set.seed(2025)
n <- 150
years_experience <- runif(n, 0, 20)
certifications <- 0.3 * years_experience + rnorm(n, 0, 1.5)
salary <- 3 * years_experience + rnorm(n, 0, 8)
plot(certifications, salary, xlab = "Number of certifications", ylab = "Salary (in $1,000s)",
     main = "A spurious correlation driven by years of experience")
```
...
::: {.callout-note collapse="false" title="Randomization is what actually fixes this"}
...
::: {.example title="Example 7.4: Designing a training-program experiment"}
...
df <- tibble(size = sizes) %>%
  mutate(
    ...
    biased_mean = map_dbl(size, ~ {
      n_high <- round(0.8 * .x)
      n_low  <- .x - n_high
      mean(c(sample(pop[pop == 15], n_high), sample(pop[pop == 5], n_low)))
    })
  ) %>%
  pivot_longer(c(random_mean, biased_mean), names_to = "type", values_to = "mean") %>%
  mutate(type = recode(type, random_mean = "Random sample", biased_mean = "Biased sample"))

ggplot(df, aes(x = size, y = mean, color = type, shape = type)) +
  geom_line() +
  geom_point(size = 2) +
  geom_hline(yintercept = true_mean, linetype = "dashed") +
  scale_x_continuous(breaks = sizes) +
  labs(title = "Random error shrinks with n; bias does not",
       x = "Sample size", y = "Sample mean", color = NULL, shape = NULL)
```
...
::: {.callout-note collapse="false" title="The classic illustration of survivorship bias"}
...
## From Association to Causation: A Checklist
