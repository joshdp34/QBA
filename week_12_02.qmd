...
::: {.example title="Example 12.7: A 7-day moving average"}
```{r}
#| echo: true
#| message: false
library(tidyverse)
...
```
Averaging over a full 7-day window cancels out the weekly seasonal swing entirely (each window contains exactly one of every day of the week), leaving a smooth line that traces the underlying trend almost as cleanly as the STL trend component from @sec-week12-01.
:::

::: {.example title="Example 12.8: Forecasting with a moving average"}
```{r}
#| echo: true
mean(tail(coffee$revenue, 7))
```
The 7-day moving average forecast for June 29 is about **\$64.69**, the average of the last full week. Compare this to @sec-week12-01's naive (\$52.60) and seasonal naive (\$54.10) forecasts: the moving average smooths out day-to-day noise, but by averaging across an entire week, it also blurs away the fact that June 29 is specifically a **Saturday**, a lower-revenue day. All three methods have a real weakness; the rest of this section builds toward something that addresses both trend and seasonality at once.
:::

::: {.callout-note collapse="false" title="Choosing the window length k"}
...
::: {.callout-important collapse="false" title="SES alone can't handle trend or seasonality"}
Because SES forecasts a flat line forever, it's only appropriate for a series with **no trend and no seasonality**, just a level that drifts randomly. The coffee shop data has both a clear trend and clear weekly seasonality (@sec-week12-01), so plain SES is the wrong tool here; it would flatten out both real patterns. This is exactly why exponential smoothing extends further, covered next.
:::

## Extending Exponential Smoothing: Trend and Season

**Holt's linear trend method** adds a second smoothed component, a trend $b_t$, updated alongside the level with its own smoothing parameter $\beta$, so forecasts can slope upward or downward rather than staying flat. **Holt-Winters' method** adds a *third* smoothed component, a seasonal pattern $S_t$, with its own smoothing parameter $\gamma$, repeating every $m$ periods. All three, SES, Holt's, and Holt-Winters, are special cases of the same underlying idea, and `fable`'s `ETS()` function (**E**rror, **T**rend, **S**eason) fits any of them the same way `NAIVE()` and `SNAIVE()` were fit in @sec-week12-01.

::: {.example title="Example 12.9: Fitting the whole family and comparing fit"}
```{r}
#| echo: true
fit <- coffee |> model(
...
::: {.example title="Example 12.10: What Holt-Winters actually estimated"}
```{r}
#| echo: true
...
::: {.example title="Example 12.11: A Holt-Winters forecast for June 29"}
```{r}
#| echo: true
...
## Does the More Sophisticated Model Actually Win?

@sec-week12-01 insisted that any fancier method has to earn its keep against the naive baselines. Following @sec-week10-02's train/test logic, hold out the last two weeks and check.

::: {.example title="Example 12.12: An honest accuracy comparison"}
```{r}
#| echo: true
...
::: {.callout-tip collapse="true" title="Why doesn't Holt-Winters win by more?"}
...
## Computing Moving Averages and Exponential Smoothing in R and Excel

::: {.example title="Example 12.13: In R and Excel"}
...
| **AICc** | A penalized fit measure for comparing models; lower is better, plays the same role here that adjusted $R^2$ played in @sec-week09-01. |
